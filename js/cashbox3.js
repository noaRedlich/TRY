(function() {
	...
	app.controller('CashboxController', ['$http', '$scope', '$rootScope', '$compile', '$timeout', 'tabs', 'clean', 'local', 'cash_global', 'GetURLParameter', 'alert', 'dates',
	function($http, $scope, $rootScope, $compile, $timeout, tabs, clean, local, cash_global, GetURLParameter, alert, dates) {
              ...
              b[i] = [];
				...
				if(i==3){
					...
					l++;
    	             ...
    	             b[i][l] = $rootScope.txtCenter("https://office.yedatop.com/officefiles/yedatop/_MlaitekPro/"+$rootScope.details_list.logo,i); //lc 21/07/2016
		             l++;
                 }
				}
				...
				if ( typeof android == 'undefined' && typeof android2 == 'undefined' && 1 == 0) {
			      ...
			      for (var i = 1; i < 4; i++) {
					...
					if ($rootScope.details_list.logo != "") {
                         ...
                         a[i][l] = $rootScope.txtCenter('<img style="max-width:170px;width:170px;" src="https://office.yedatop.com/officefiles/yedatop/_MlaitekPro/'+$rootScope.details_list.logo+'"/>',i);
                         l++;
                     }
					 ...
					 if ($rootScope.details_list.name != "") {
                            ...
                            if (transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {
                                ...
                                for (j = 0; j < products.length; j++) {
                                    product = products[j];
                                    cat_vat = $.grep($rootScope.catJson,function(e){ return e.ID== product["department"] || e.ID== product["category"]; });
                                    if (typeof cat_vat[0] != 'undefined' && cat_vat[0]["VatFree"] == 1) {
										vat_free = vat_free + product["Amount"] * product["SalePrice"] - product["Discount"] - product["discount_sale"];
									}
                            ...
                            for (var key in payments) {
                                if (payments.hasOwnProperty(key))
                                    for (j = 0; j < payments[key].length; j++) {
                                        ...
                                        if (payment["type"] != 'מזומן' && payment["type"] != 'המחאה' && payment["type"] != 'הקפה') {
                            ...
                            if (transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {
                                ...
                                if(free_vat==1){
                                	...
                                	a[i][l] = $rootScope.itemLine("        חייב מע\"מ:", ($scope.cashes['cash_num' + num].amountbeforevat - vat_free * 100 / 117).toFixed(2), i);
	                                l++;
	                                if (vat_free > 0) {
										a[i][l] = $rootScope.itemLine("        פטור מע\"מ:", vat_free, i);
										l++;
									}
	                                a[i][l] = $rootScope.itemLine("        סה\"כ מע\"מ:", ($scope.cashes['cash_num' + num].vat - vat_free * 17 / 117).toFixed(2), i);
	                                l++;
                                }
                            }
