			...
			$http.post('inc/functions.php',  $.param({id_worker: n,stat:"start",stock:stock,show_days_report:show_days_report}, {async: true}), {
	        	headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
					...
					if($rootScope.details_list.print_setting){	
		  				var b = [];		 
						b[0]= $rootScope.txtCenter('ביצוע כניסה');
						b[1]= $rootScope.txtCenter("שם עובד: "+ worker_name1);		
					  	b[2]= $rootScope.txtCenter("עובד מס': "+ worker_num1);
					    b[3]= $rootScope.txtCenter("תאריך: "+ $('#current_date').html());
					    b[4]= $rootScope.txtCenter("שעה: "+ $('#curr_date').html());
					    b[5]= $rootScope.txtCenter("עבודה נעימה!!!");
		...
		if($rootScope.SearchWorker.length!=0&&$rootScope.SearchWorker!=undefined && $rootScope.SearchWorker.is_clock==1){	
				...
				$http.post('inc/functions.php',  $.param({id_worker: n,stat:"end",stock:stock}), {
		        	headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
						...
						if($rootScope.details_list.print_setting){	
							var b = [];		 
							b[0]= $rootScope.txtCenter('ביצוע יציאה');
							b[1]= $rootScope.txtCenter("שם עובד: "+ worker_name1);		
						  	b[2]= $rootScope.txtCenter("עובד מס': "+ worker_num1);
						    b[3]= $rootScope.txtCenter("תאריך: "+ $('#current_date').html());
						    b[4]= $rootScope.txtCenter("שעה: "+ $('#curr_date').html());
						    b[5]= $rootScope.txtCenter("המשך יום טוב!!!");
						    printPrinter(b);
						 }
						setTimeout(function () {
					  $("div[name=clock_display2]").html("");
