...
//
	...
	this.add_style=function(picture){
			if(picture!=''){
					...
					return "width:26px!important;height:25px!important;";
				}
		};
	this.add_class=function(picture){
			if(picture!=''){
					return "";
				}
				else{
    ...
    app.controller('CashProdController', ['$http','$scope', '$rootScope','local','cash_global','tabs','alert', function($http,$scope,$rootScope, local,cash_global,tabs,alert){
		...
		$rootScope.check_discounts = function(){
			...
			for(var j = 0; j < $rootScope.discounts.length; j++){
				...
				var discount_prd = [];
				var cnt_minus=0;
				var discount_prd_minus = [];
				for(var i = 0; i < cash_prd.products.length; i++){
		       		 var product = cash_prd.products[i];
		        	if(discount.items.indexOf(parseInt(product.ID)) != -1 ){
			        	if( parseFloat(product.SalePrice) > 0){
		        			cnt+=parseFloat(product.Amount);
		        			discount_prd.push(product);
			        	}
			        	else{
			        		cnt_minus+=parseFloat(product.Amount);
		        			discount_prd_minus.push(product);
			        	}
		        	}
		        }
		        var ind = 0;
		        ...
		        switch(discount.type) {
		        	case "1"://n+n
		        		...
		        		break;
		        	case "2"://n ב nה
						var op2_arr = discount.op2.split(";");
						var discount_op1 = parseInt(discount.op1);
		        		if(cnt >= discount_op1){
		        	    	discount_prd.sort($scope.compare);
		        	    	for(var x = 0; x < discount_prd.length; x++){
		        	    		discount_amount = 0;
		        	    		for (var z =0; z < discount_prd[x].Amount; z++){
		        	    			ind++;
									for(var op1_i = discount_op1; op1_i < discount_op1 + op2_arr.length - 1; op1_i++) {
										if (ind == op1_i) {
											discount_amount = discount_amount + (discount_prd[x].SalePrice - discount_prd[x].Discount) - parseInt(op2_arr[op1_i-discount_op1]);
										}
									}
									if (ind >= op1_i) {
										discount_amount = discount_amount + (discount_prd[x].SalePrice - discount_prd[x].Discount) - parseInt(op2_arr[op1_i-discount_op1]);
		        ...
		        switch(discount.type) {
		        	case "1"://n+n
		        	    if(cnt_minus > parseInt(discount.op1)){
		        	    	discount_prd_minus.sort($scope.compare);
		        	    	for(var x = discount_prd_minus.length-1; x >= 0; x--){
		        	    		discount_amount = 0;
		        	    		for (var z =0; z < discount_prd_minus[x].Amount; z++){
		        	    			if(ind == parseInt(discount.op1)+parseInt(discount.op2)){
		        	    				ind = 0;
		        	    			}
		        	    			ind++;
		        	    			if(ind > parseInt(discount.op1) && ind <= parseInt(discount.op1)+parseInt(discount.op2)){
		        	    				discount_amount = discount_amount + (discount_prd_minus[x].SalePrice - discount_prd_minus[x].Discount);
		        	    			}
		        	    		}
		        	    		if(discount_amount < 0){
		        	    			$rootScope.discount_sale = $rootScope.discount_sale + discount_amount;
		        	    			prd = cash_prd.products.filter(function(obj){
		        	    				return obj.index == discount_prd_minus[x].index;
		        	    			});
		        	    			prd[0].discount_sale = prd[0].discount_sale + discount_amount;
			        	    	}
			        	    }
		        		...
		        		if(cnt_minus >= discount_op1){
		        	    	...
		        	    	for(var x = discount_prd_minus.length-1; x >= 0; x--){
		        	    		...
		        	    		for (var z =0; z < discount_prd_minus[x].Amount; z++){
									...
									for(var op1_i = discount_op1; op1_i < discount_op1 + op2_arr.length - 1; op1_i++) {
										if (ind == op1_i) {
											discount_amount = discount_amount + (discount_prd_minus[x].SalePrice - discount_prd_minus[x].Discount) + parseInt(op2_arr[op1_i-discount_op1]);
										}
									}
									if (ind >= op1_i) {
										discount_amount = discount_amount + (discount_prd_minus[x].SalePrice - discount_prd_minus[x].Discount) + parseInt(op2_arr[op1_i-discount_op1]);
		        		...
		        		if(cnt_minus >= parseInt(discount.op1)){
		        	    	...
		        	    	for(var x =  discount_prd_minus.length-1; x >=0; x--){
		        	    		...
		        	    		for (var z =0; z < discount_prd_minus[x].Amount; z++){
		        	    			...
		        	    			ind++;
		        	    			if(ind == parseInt(discount.op1)){
		        	    				discount_amount = discount_amount + (discount_prd_minus[x].SalePrice - discount_prd_minus[x].Discount) * parseInt(discount.op2) / 100;
		        	    			}
		        	    		}
		        	    		if(discount_amount < 0){
		        	    			$rootScope.discount_sale = $rootScope.discount_sale + discount_amount;
	    ...
	    this.add_new_cart = function(){
	    	...
	    	if($('.input_result input[name=Input]').val()!=''&&$('.input_result input[name=Input]').val()!='undefined'){
	    		if($(".add_worker.popup_pay  ").css("display")=="block"||$(".pop_anacha ").css("display")=="block"||$(".pop_tip ").css("display")=="block" || ($rootScope.count_index !=undefined && $rootScope.count_index != -1)){
	    			//$(".mezuman_sum").val($('.input_result input[name=Input]').val())
	    		}
	    		else{
	    			if (this.permission_global_item()){
...
/*		this.add_general_weight_cart = function(){
      ...
      cashlog('add_general_weight_cart'); // here code log
            ...
            cashlog('add_general_weight_cart', $('.input_result input[name=Input]').val(), "$('.input_result input[name=Input]').val()"); // here code log
            ...
            if($rootScope.refund==0){
              ...
              cashlog('add_general_weight_cart', {
                "$rootScope.numprod_hova": $rootScope.numprod_hova,
              "$rootScope.numprod_hova_cash": $rootScope.numprod_hova_cash
							}, '$rootScope.refund==0'); // here code log
					   }
					   else{
					   		$rootScope.numprod_zchut++;
					   		$rootScope.numprod_zchut_cash++;
            ...
            // cashlog('add_general_weight_cart', {
            //   'typeof window.external.getWeight': typeof window.external.getWeight
            // }, 'window.external'); // here code log
					...
					writelog("הוספת פריט כללי בסכום: "+price);
          ashlog('add_general_weight_cart', 'before x');
					$rootScope.index++;
			   		var x={
			   			"Amount": amount,
						"ID":"0",
            ...
            cashlog('add_general_weight_cart', x); // here code log
      ...
      this.add_general_weight_cart = function(){ // VatWeight
        ...
        if($('.input_result input[name=Input]').val()!=''&&$('.input_result input[name=Input]').val()!='undefined'){
          ...
          if($(".add_worker.popup_pay  ").css("display")=="block"||$(".pop_anacha ").css("display")=="block"||$(".pop_tip ").css("display")=="block"){
            ...
            if($rootScope.refund==0){
              ...
              cashlog('add_general_weight_cart', {
                'typeof window.external.getWeight': typeof window.external.getWeight
              }, '$rootScope.refund==0 -> window.external'); // here code log
            }
            else{
              $rootScope.numprod_zchut++;
              $rootScope.numprod_zchut_cash++;
            ...
            if(typeof window.external != 'undefined' && typeof window.external.getWeight != 'undefined' && typeof window.external.getWeight != 'unknown'){ // here code checking
            	cashlog('add_general_weight_cart', 'in external'); // here code log
              amount= window.external.getWeight();
              if(amount <= 0) amount = 1; // here code
            }
            else{
            	if(typeof csharp != 'undefined' && typeof csharp.getWeight != 'undefined' && typeof csharp.getWeight != 'unknown'){ // here code checking
	            	cashlog('add_general_weight_cart', 'in external'); // here code log
	              amount= csharp.getWeight();
	              if(amount <= 0) amount = 1; // here code
	            }
	            else{
	              cashlog('add_general_weight_cart', 'in else external'); // here code log
	              amount = 1;
             	}
            }
      ...
      // noVatWeight function
			this.add_general_weight_cart_no_vat = function(){ // noVatWeight

				console.log('add_general_weight_cart_no_vat')

        cashlog('add_general_weight_cart_no_vat'); // here code log
        ...
        if($('.input_result input[name=Input]').val()!=''&&$('.input_result input[name=Input]').val()!='undefined'){
          ...
          if($(".add_worker.popup_pay  ").css("display")=="block"||$(".pop_anacha ").css("display")=="block"||$(".pop_tip ").css("display")=="block"){
            //$(".mezuman_sum").val($('.input_result input[name=Input]').val())
          	...
          	cashlog('add_general_weight_cart_no_vat', $('.input_result input[name=Input]').val(), "$('.input_result input[name=Input]').val()"); // here code log
            ...
            if($rootScope.refund==0){
              ...
              cashlog('add_general_weight_cart_no_vat', {
                'typeof window.external.getWeight': typeof window.external.getWeight
              }, '$rootScope.refund==0 -> window.external'); // here code log
            }
            else{
            ...
            if($rootScope.refund==1){
              var price=(parseFloat($('.input_result input[name=Input]').val())).toFixed(2); // noVatWeight change
              // var price=(parseFloat($('.input_result input[name=Input]').val())*-1).toFixed(2); // noVatWeight change
              $('.zicuy').toggleClass("zicuy_zctive");
              //$rootScope.refund=0;
            }
            else{
            ...
            if(typeof window.external != 'undefined' && typeof window.external.getWeight != 'undefined' && typeof window.external.getWeight != 'unknown'){ // here code checking
              cashlog('add_general_weight_cart_no_vat', 'in external'); // here code log
              amount= window.external.getWeight();
              if(amount <= 0) amount = 1; // here code
            }
            else{
            	if(typeof csharp != 'undefined' && typeof csharp.getWeight != 'undefined' && typeof csharp.getWeight != 'unknown'){ // here code checking
	              ...
	              cashlog('add_general_weight_cart_no_vat', 'in else external'); // here code log
	              amount = 1;
	             }
            }
            ...
            cashlog('add_general_weight_cart_no_vat', x); // here code log
            ...
            $rootScope.amount_maam = 0; // here code: vat = 0
            $rootScope.amount_out= (parseFloat($rootScope.amount)).toFixed(2);

             $rootScope.original_afterprod=$scope.getTotal(); // remove for no vat... 11/09/2019
...
/*sk 11/10 change this function and i put the source in remark*/
  ...
  // here code cashlog
  function cashlog(functionName, data, notation){
    var date = new Date();
