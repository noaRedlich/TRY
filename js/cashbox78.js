(function () {
	...
	app.directive("pause", function () {
		return {
			restrict: 'E',
			templateUrl: "inc/pause.html?ver9"
		};
	});
			...
			return {
				clean: function () {
						...
						for (var key in cash_global.payments_type) {
							if (cash_global.payments_type.hasOwnProperty(key)) {
								for (i = 0; i < payments[key].length; i++) {
									...
									if (key != 'akafa' && key != 'shovarzicuy'&&key!='points') {/*sk 03/09 add the conditiion*///בהקפה לא מוסיפים את הכסום לסכום הכללי
									...
									if (payment.amount < 0) {
										...
										if (key != 'akafa' && key != 'shovarzicuy'&&key!='points') {/*sk 07/09*/
					...
					if ($rootScope.cash_num_pause != -1) {
						...
						local.setLocal('cashes', cash_global.cashes);
	...
	app.controller('CashboxController', ['$http', '$scope', '$rootScope', '$compile', '$timeout', 'tabs', 'clean', 'local', 'cash_global', 'GetURLParameter', 'alert', 'dates', 'total',
		function ($http, $scope, $rootScope, $compile, $timeout, tabs, clean, local, cash_global, GetURLParameter, alert, dates, total) {
			...
			$scope.isclean = function () {
				...
				// $rootScope.cardSign = 0; // here code: card sign
				if ($rootScope.amount != 0 || $rootScope.countprod != 0 || ($rootScope.SearchClient != undefined && $rootScope.SearchClient.length != 0)) {
					...
					if ($rootScope.cash_num_pause != -1 && $rootScope.cash_num_pause != undefined) {
						$scope.call_alert_site('האם ברצונך למחוק את החשבון? החשבון נמצא בהשהייה', 0, '');
						 delete cash_global.cashes['cash_num'+$rootScope.cash_num_pause+'*'];
			             delete $scope.cashes['cash_num'+$rootScope.cash_num_pause+'*'];
...
$scope.printTemp1 = function (id) {
	$rootScope.cash_num=id;
 	$rootScope.end_cash(2);
 		// if($rootScope.details_list.print_check ==1 ){
 	   	    // var newWindow = window.open('', '', 'top='+(window.screenTop+5)+',left='+(window.screenLeft+20)+',height=500,width=800,resizable=yes,scrollbars=yes,status=yes');
            // newWindow.document.write( a4_chesh);
					// }
					// else{
					// var print='	<input type="button" id="print_button" value=" הדפס " style="cursor:hand;padding:0 0 0 10;"onclick="window.print();"/>';
 	   	    // var newWindow = window.open('', '', 'top='+(window.screenTop+5)+',left='+(window.screenLeft+20)+',height=500,width=800,resizable=yes,scrollbars=yes,status=yes');
            // newWindow.document.write(JSON.parse(print_save.html)[0]);
					// }
    };
 $scope.printTemp = function () {	
    $rootScope.end_cash(1);
 	$rootScope.end_cash(2);
 		// if($rootScope.details_list.print_check ==1 ){
 	   	    // var newWindow = window.open('', '', 'top='+(window.screenTop+5)+',left='+(window.screenLeft+20)+',height=500,width=800,resizable=yes,scrollbars=yes,status=yes');
            // newWindow.document.write( a4_chesh);
...
$rootScope.get_a4_chesh =function(ispause){
	var num = parseFloat($rootScope.cash_num) - 1;
	 if(ispause==2){
						num = num + "*";
					}
var a4_string = "<html dir='rtl'>\
<head></head>\
<body style='display:inline;'>\
    <meta http-equiv='Content-Type' content='text/html; charset=UTF-8'>\
    <link href='https://office1.yedatop.com/themes/blue/style.css' rel='stylesheet' type='text/css'>\
    <base target='_self'>\
    <title>H - YedaTop</title>\
    <div>\
        <pre style='text-align:left;direction:ltr;'></pre>\
        <div dir='rtl'>\
        </div>\
        <table id='REPORTTABLE' dir='rtl' width='100%' border='0' cellpadding='2'>\
            <style>\
                body {\
                    width: 18cm !important;\
                    height: 26.7cm !important;\
                ...
                <tr valign='top'>\
                    <td width='99%'>\
                        <div class='company-name'>\
                            "+$rootScope.details_list.name+" </div>\
                        <div>\
                            ע.מ./ח.פ.: "+$rootScope.details_list.tz+"\
                        </div>\
                    </td>\
                    <td width='1%'>\
                         <img src='https://kupa.yedatop.com/officefiles/" + user_name + "/_MlaitekPro/" + $rootScope.details_list.logo +"' height='70'>\
                    </td>\
                </tr>\
            </tbody>\
        </table>\
        ...
        <table width='100%'>\
            <tbody>\
                <tr>\
                    <td width='33%'>\
                        <fieldset class='box'>\
                            <legend>פרטי מסמך</legend>\
                            <div>\
                                #"+$scope.cashes['cash_num' + num].cash_kupanum+"\
                                <br>\
                                תאריך:"+$('#current_date').html()+" שעה:" +$('#curr_date').html()+"\
                                 </div>\
                        </fieldset>\
                    </td>\
                    <td nowrap='' width='33%'>\
                        <fieldset class='box'>\
                           ...
                           a4_string = a4_string+ "לקוח/ה:" + " " + $scope.cashes['cash_num' + num].SearchClient.ClientNum + " " + $scope.cashes['cash_num' + num].SearchClient.SupplierName;
                           if($scope.cashes['cash_num' + num].SearchClient.BusinessNum != "" && $scope.cashes['cash_num' + num].SearchClient.BusinessNum!= undefined){
                           	   a4_string = a4_string+"<br>"+ "עוסק מורשה:" + " " + $scope.cashes['cash_num' + num].SearchClient.BusinessNum;

                           }
                            // if($scope.cashes['cash_num' + num].SearchClient.CellPhone != "" && $scope.cashes['cash_num' + num].SearchClient.CellPhone!= undefined){
                           	   // a4_string = a4_string+"<br>"+ "טלפון נייד:" + " " + $scope.cashes['cash_num' + num].SearchClient.CellPhone;
// 
                           // }
                           if($scope.cashes['cash_num' + num].SearchClient.Address != "" && $scope.cashes['cash_num' + num].SearchClient.Address!= undefined){
                           	   a4_string = a4_string+"<br>"+ "כתובת:" + " " + $scope.cashes['cash_num' + num].SearchClient.Address;

                           }
                            }
                            else{
                            	  if ($scope.cashes['cash_num' +num]['customer_m_name'] != '') {
                           a4_string = a4_string+ "לקוח/ה:" + " " + $scope.cashes['cash_num' +num]['customer_m_name']+"<br>";
                            	  }
                            	  if ($scope.cashes['cash_num' +num]['customer_m_phone'] != '') {
                           a4_string = a4_string+ "טלפון נייד:" + " " + $scope.cashes['cash_num' +num]['customer_m_phone']+ "<br>";
                            	  }
                            	  if ($scope.cashes['cash_num' +num]['customer_m_car'] != '') {
                           a4_string = a4_string+"מס רכב:" + " " + $scope.cashes['cash_num' +num]['customer_m_car'];
                            	  }
                            }
                            a4_string = a4_string +"</div>\
                        </fieldset>\
                    </td>\
                    <td width='33%'>\
                        <fieldset class='box'>\
                            <legend>מאת</legend>\
                            <div>"+$rootScope.details_list.name+"<br>\
                            ע.מ./ח.פ.: "+$rootScope.details_list.tz;
                            if ($rootScope.details_list.address != "" && $rootScope.details_list.print_address == '1') {
                            a4_string = a4_string+"<br>" + $rootScope.details_list.address;
                        }
                        if ($rootScope.details_list.mikud != "" && $rootScope.details_list.print_mikud == '1') {
                            a4_string = a4_string+"<br>" +" מיקוד:" + $rootScope.details_list.mikud;
                        }
                        if ($rootScope.details_list.tel != "" && $rootScope.details_list.print_tel == '1') {
                             a4_string = a4_string +"<br>"+" טלפון:" + $rootScope.details_list.tel;
                        }
                        if ($rootScope.details_list.fax != "" && $rootScope.details_list.print_fax == '1') {
                           a4_string = a4_string +"<br>"+" פקס:" + $rootScope.details_list.fax;
                              }
                        if ($rootScope.details_list.mail != "" && $rootScope.details_list.print_mail == '1') {
                           
                            a4_string = a4_string+"<br>"+" דוא\"ל:" + $rootScope.details_list.mail;
                            }
                        if ($rootScope.details_list.web != "" && $rootScope.details_list.print_web == '1') {
                            a4_string = a4_string +"<br>"+" אתר אינטרנט:" + $rootScope.details_list.web;
                            }
...
a4_string= a4_string+"                            </div>\
        ...
        <div class='spacer'></div>\
        <center>\
            <div class='doc-name'>"+$scope.cashes['cash_num' + num].cash_kupanum+" ";
            
             if ($scope.cashes['cash_num' + num].prepaid2 == 1) {
                            a4_string = a4_string +"קבלה על טעינת כרטיס";
                        }
                        else {
                            if (transtype == 0) {
                                if (amount > 0) {
                                	if(ispause==2){
                                        a4_string = a4_string +"חשבון זמני";
                                    	}
                                    	else{
                                    		 if (takbul == 1) {  
                                        a4_string = a4_string +"חשבונית מס קבלה - מקור";
                                        } else {
                                        
                                        a4_string = a4_string +"מקור- חשבונית מס";
                                        }
                                    	}
                                   
                                } else {
                                   a4_string = a4_string +"הודעת זיכוי";
                                    }
                            } else {
                            	if(ispause==2){
                                        a4_string = a4_string +"חשבון זמני";
                                    		 ...
                                    		 if (takbul == 1) {
                                    a4_string = a4_string +"קבלה";
                                   } else {
                                    if (shovarzicuy_l == 0 && points_l==0) {
                                       a4_string = a4_string +"חשבונית מס - מקור";
                                        } else {
                                        if (shovar_l == 0) {
                                            
                                            a4_string = a4_string +"מקור- חשבונית מס קבלה";
                                            } else {
                                           a4_string = a4_string +"אישור תשלום";
                                           
                                        }
                                    }
                                }
                                a4_string = a4_string +"תשלום חוב";
                                    	}
        ...
        <div class='spacer'></div>\
        <div style='text-align: right;'>מוצרים\
        </div>\
...
for (j = 0; j < products.length; j++) {
                                ...
                                a4_string=a4_string+"\
                <tr>\
                    <td width='20%' nowrap='' id='TD587'>"+product["BarCode"]+"</td>\
                    <td width='20%' nowrap=''>"+product["Title"]+"</td>\
                    <td width='7%' dir='ltr' align='right'>"+product["Amount"]+"</td>\
                    <td width='8%' dir='ltr' align='right'>"+product["SalePrice"]+"</td>\
                    <td width='15%' dir='ltr' align='right'>"+product["Amount"]*product["SalePrice"]+"</td>\
                    <td width='17%'>";
                    if (parseFloat(product["Discount"]) != 0) {
                                    sumd += parseFloat(product["Discount"]);
                                    a4_string+= "-" + product["Discount"];
                                    }
                                    if (parseFloat(product["discount_sale"]) != 0) {
                                    sumd += parseFloat(product["discount_sale"]);
                                    a4_string+= "-" + product["discount_sale"];
                                  }
                    a4_string = a4_string+"</td>\
                    <td width='13%'><span dir='ltr'>"+(product["Amount"]*product["SalePrice"]-product["Discount"]-product["discount_sale"])+"</span></td>\
                </tr>";
               }
              }
               a4_string= a4_string + "\
        ...
        <table width='100%' border='1' style='border-collapse:collapse'>\
             ...
             a4_string= a4_string + "   <tr>";
                   payment = payments[key][j];
                                    ...
                                    if (payment["type"] != 'מזומן' && payment["type"] != 'המחאה' && payment["type"] != 'הקפה' && payment["type"] != 'פריפייד') {
                                    	a4_string = a4_string+"<td valign='top'>"+payment["type"]+"</td>\
                    <td valign='top'>"+payment.amount+"</td>";
                    if(key != 'credit'){
                    	a4_string = a4_string+"<td id='informationField_1'>&nbsp;</td>";
                    	}
                     else {
                     	a4_string = a4_string+"<td id='informationField_1'>מס' כרטיס: "+payment.ashray_numcard.substr(payment.ashray_numcard.length - 4)+", מס' אישור: "+payment.approvalNumber;
                                        
                                        if (payment.pay_num > 0) {
                                        	a4_string = a4_string+", מס' תשלומים: "+payment.pay_num;
                                            if (payment.pay_first > 0) {
                                            	a4_string = a4_string+", תשלום ראשון: "+payment.pay_first;
                                            }
                                        }
                                        else {
                                            if (payment.ashray_f_credit) {
                                                a4_string = a4_string+", מס' תשלומי קרדיט: "+ payment.creditbrand;
                                            }
                                        }
                                        a4_string = a4_string+"<\td>";
                                    }
                                    	...
                                    	a4_string = a4_string+"<td valign='top'>"+payment["type"]+"</td>\
                    <td valign='top'>"+payment.amount+"</td>\
                    <td id='informationField_1'>מספר כרטיס: "+ payment.prepaid_num+", יתרה בכרטיס: "+parseFloat(payment.balance) - parseFloat(payment.amount)+"</td>";
                                       
                                    }
                                    else if (payment["type"] != 'המחאה' && payment["type"] != 'הקפה') {
                                    	a4_string = a4_string+"<td valign='top'>התקבל במזומן</td>\
                    <td valign='top'>"+payment.amount+"</td>\
                    <td id='informationField_1'>&nbsp;</td>";
                                    }
                                    else {
                                        if (payment["type"] != 'הקפה') {
                                        	a4_string = a4_string+"<td valign='top'>"+payment["type"]+"</td>\
                    <td valign='top'>"+payment.amount+"</td>\
                    <td id='informationField_1'>מספר המחאה: "+ payment.chequenumber+"</td>";
            ...
            a4_string= a4_string + " </tbody>\
        </table>\
    </div>\
    <div style='float: right;margin-top: 15px;'>\
    </div>\
    <table border='1' style='border-collapse:collapse;margin-right: auto;margin-top: 10px;'>\
        <tbody>";
        if (typeof $scope.cashes['cash_num' + num].original_afterprod != 'undefined') {
                            a4_string= a4_string + "<tr>\
                <td nowrap=''><b>סכום ביניים:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].original_afterprod+"</b></td>\
            </tr>";
                        }
                         if ($scope.cashes['cash_num' + num].finalamount - $scope.cashes['cash_num' + num].original_afterprod > 0 && transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {
                             a4_string= a4_string + "<tr>\
                <td nowrap=''><b>הנחת חשבון:</b></td>\
                <td><b>"+($scope.cashes['cash_num' + num].finalamount - $scope.cashes['cash_num' + num].original_afterprod).toFixed(2)+"</b></td>\
            </tr>";
                        }
                        if ($scope.cashes['cash_num' + num].round > 0) {
                             a4_string= a4_string + "<tr>\
                <td nowrap=''><b>עיגול אגורות:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].round +"</b></td>\
            </tr>";
            }
			...
			a4_string= a4_string + "<tr>\
                <td nowrap=''><b>עודף:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].change +"</b></td>\
            </tr>";
             }
             if (transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {

                            var free_vat = $rootScope.details_list.free_vat;
                            if (free_vat == 1||parseFloat($scope.cashes['cash_num' + num].vat)==0) {
                            	a4_string= a4_string + "<tr>\
                <td nowrap=''><b>חייב מע\"מ:</b></td>\
                <td><b>פטור מע\"מ</b></td>\
            </tr>";
                            }
                            else {
                                if ($scope.cashes['cash_num' + num].vat_free > 0) {
                                    a4_string= a4_string + "<tr>\
                <td nowrap=''><b>חייב מע''מ:</b></td>\
                <td><b>"+(parseFloat($scope.cashes['cash_num' + num].amountbeforevat - $scope.cashes['cash_num' + num].vat_free).toFixed(2))+"</b></td>\
            </tr>";
              a4_string= a4_string + "<tr>\
                <td nowrap=''><b>פטור מע''מ:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].vat_free+"</b></td>\
            </tr>";
            a4_string= a4_string + "<tr>\
                <td nowrap=''><b>סה''כ מע''מ:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].vat+"</b></td>\
            </tr>";
                                    ...
                                    a4_string= a4_string + "<tr>\
                <td nowrap=''><b>חייב מע''מ:</b></td>\
                <td><b>"+$scope.cashes['cash_num' + num].amountbeforevat +"</b></td>\
            </tr>";
            ...
            a4_string= a4_string +" <tr>\
                <td nowrap=''><b>סה''כ שולם:</b></td>\
                <td nowrap=''><b>"+totalf+"</b></td>\
            </tr>\
        </tbody>\
    </table>\
    <div class='spacer'></div>\
        ...
        <tbody>\
            <tr valign='top'>\
                ...
                <td align='left'>\
                    ...
                    <img style='max-width:170px;width:170px;'\
                        src='https://office1.yedatop.com/modules/stock/cashbox_fe/inc/barcode.php?height=50&amp;barcode=000890001'>\
                </td>\
            </tr>\
        </tbody>\
    </table>\
...
return a4_string;
			...
			$rootScope.end_cash = function (ispause) {
				...
				if (ispause == 1) {
					...
					if (ispause == 2) {
					writelog("השהיית והדפסת עיסקה: " + $rootScope.cash_num)
				}
				...
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['payments'] = cash_global.payments_type;
				...
				for (var i = 0; i < cash_global.products.length; i++) {
					...
					if(product.department=="-2"){
						firstamount_withoutmaam += ((product.SalePrice - product.Discount) * product.Amount) * (100 - discountperc) / 100;

					}
					else{
					for (index = 0; index < $scope.catJson.length; ++index) {
						if ($scope.catJson[index].ID == product.department && $scope.catJson[index].VatFree == "1") {
							firstamount_withoutmaam += ((product.SalePrice - product.Discount) * product.Amount) * (100 - discountperc) / 100;
						}
					}
					}
				}
				$rootScope.amount_maam = ($rootScope.amount_maam - ((firstamount_withoutmaam * 17) / 117)).toFixed(2);
				$rootScope.amount_out = (parseFloat($rootScope.amount_out) + ((firstamount_withoutmaam * 17) / 117)).toFixed(2);
				...
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['vat_free'] = firstamount_withoutmaam.toFixed(2);
				...
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['customer_m_name'] = $("#add_customer_m").val();//שם לקוח מזדמן
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['customer_m_phone'] = $("#add_customer_m_phone").val();//טלפון לקוח מזדמן
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['customer_m_car'] = $("#add_customer_m_car").val();//רכב לקוח מזדמן
				$scope.cashes['cash_num' + $rootScope.cash_num + pause_string]['customer_display_name'] = (typeof $rootScope.SearchClient.SupplierName == 'undefined')?$("#add_customer_m").val(): $rootScope.SearchClient.SupplierName;//שם לקוח מזדמן
				...
				if ($rootScope.action != 'del_cash' && ispause != 1) {
					$rootScope.save_final_tash(firstamount_withoutmaam, cash_global);
				}
				if(ispause==1){
					$rootScope.call_clean();
				}
				...
				if (ispause != 1) {
					...
					for (var i = 1; i < 4; i++) {
						...
						if ($rootScope.details_list.logo != "") {
							...
							// a[i][l] = $rootScope.txtCenter('<img style="max-width:170px;width:170px;" src="http:kupa.yedatop.com/officefiles/' + user_name + '/_MlaitekPro/' + $rootScope.details_list.logo + '"/>', i);
							a[i][l] = $rootScope.txtCenter('<img style="max-width:170px;width:170px;" src="https://kupa.yedatop.com/officefiles/' + user_name + '/_MlaitekPro/' + $rootScope.details_list.logo + '"/>', i);
							l++;
						}
						...
						points_l = payments["points"].length;
						...
						if ($scope.cashes['cash_num' + num].prepaid2 == 1) {
							...
							if (transtype == 0) {
								if (amount > 0) {
									if (takbul == 1) {
											if(ispause==2){
											a[i][l] = $rootScope.txtCenter("חשבון זמני", i);
										l++;
										}
										else{
												//a[i][l] = "";
						...
						if (transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {
							...
							//l++;
							tip = "";
							// if(products.length = 1 && $rootScope.discount_sale > 0){
									// sumd += parseFloat($rootScope.discount_sale);
									// a[i][l] = $rootScope.itemLine("    " + products[0]["discount_desc"] + " ", "-" + $rootScope.discount_sale, i);
							// }
							for (j = 0; j < products.length; j++) {
								product = products[j];
								if (product["category"] == "9999") {
								...
								//הוספה לחשבונית מידע על מבצע שקיים בחשבון
								if (parseFloat(product["discount_sale"]) == 0 && $rootScope.discount_sale > 0 && products.length == 1) {
									sumd += parseFloat($rootScope.discount_sale);
									a[i][l] = $rootScope.itemLine("    " + product["discount_desc"] + " ", "-" + $rootScope.discount_sale, i);
									l++;
								}
						...
						if (transtype == 0 && $scope.cashes['cash_num' + num].prepaid2 == 0) {
							...
							if (free_vat == 1||parseFloat($scope.cashes['cash_num' + num].vat)==0) {
								...
								if ($scope.cashes['cash_num' + num].vat_free > 0) {
									a[i][l] = $rootScope.itemLine("        חייב מע\"מ:", parseFloat($scope.cashes['cash_num' + num].amountbeforevat - $scope.cashes['cash_num' + num].vat_free).toFixed(2), i);
									l++;
									a[i][l] = $rootScope.itemLine("        פטור מע\"מ:", $scope.cashes['cash_num' + num].vat_free, i);
									l++;
						...
						// here code: card sign 
						/*
													  if($rootScope.cardSign){
														  a[i][l] = $rootScope.itemLine("טעינת כרטיס מתנה", '', i);
														  l++;
														  a[i][l] = $rootScope.itemLine("מס. כרטיס", "<cardNumber>" + $rootScope.cardNum + "</cardNumber>", i);
														  l++;
													  }
						*/
						...
						if ($scope.cashes['cash_num' + num].prepaid2 == 0 && transtype == 0 && totalf > 0&&$scope.cashes['cash_num' + num].SearchClient.SupplierName != undefined) {
						var client_num = 		$scope.cashes['cash_num' + num].SearchClient.ClientNum ;
							$.ajax({
						type: "POST",
						async: false,
						url: "inc/functions.php",
						data: {
							client_points : parseFloat(totalf),
						...
						success: function (data) {
							if (parseFloat(data) > 0) {
									a[i][l] = "";
									l++;
									a[i][l] = $rootScope.txtCenter("לקוח/ה יקר/ה בקנייה זו צברת " + parseFloat(data).toFixed(2) + " נקודות", i);
									l++;
								}
						},
						error: function () {
						...
						if($scope.cashes['cash_num' + num].SearchClient.BusinessNum != "" && $scope.cashes['cash_num' + num].SearchClient.BusinessNum!= undefined){
							...
							a[i][l] = $rootScope.txtCenter("עוסק מורשה:" + " " + $scope.cashes['cash_num' + num].SearchClient.BusinessNum,i);
	l++;
	}
	// if($scope.cashes['cash_num' + num].SearchClient.CellPhone != "" && $scope.cashes['cash_num' + num].SearchClient.CellPhone!= undefined){
	// a[i][l] = $rootScope.txtCenter("טלפון נייד:" + " " + $scope.cashes['cash_num' + num].SearchClient.CellPhone,i);
	// l++;
// 
	// }
	if($scope.cashes['cash_num' + num].SearchClient.Address != "" && $scope.cashes['cash_num' + num].SearchClient.Address!= undefined){
	a[i][l] = $rootScope.txtCenter("כתובת:" + " " + $scope.cashes['cash_num' + num].SearchClient.Address,i);
	l++;
							...
							// p=p+$rootScope.txtCenter("קופאי/ת:"+" "+$rootScope.SearchWorker.WorkerNum+" "+$rootScope.SearchWorker.SupplierName,i);
							   ...
							   if ($scope.cashes['cash_num' +num]['customer_m_name'] != '') {
                            	   	a[i][l] = $rootScope.txtCenter("לקוח/ה:" + " " + $scope.cashes['cash_num' +num]['customer_m_name'], i);
							l++;
                            	  }
                            	  if ($scope.cashes['cash_num' +num]['customer_m_phone'] != '') {
                            	  	a[i][l] = $rootScope.txtCenter("טלפון:" + " " + $scope.cashes['cash_num' +num]['customer_m_phone'], i);
							l++;
                            	  }
                            	  if ($scope.cashes['cash_num' +num]['customer_m_car'] != '') {
                            	  	a[i][l] = $rootScope.txtCenter("מס' רכב:" + " " + $scope.cashes['cash_num' +num]['customer_m_car'], i);
							l++;
                            	  }                           	  
						}
						...
						a[i][l] = $rootScope.txtCenter('<img style="max-width:170px;width:170px;" src="https://kupa.yedatop.com/modules/stock/cashbox_fe/inc/barcode.php?height=50&barcode=' + $scope.cashes['cash_num' + num].cash_kupanum + '"/>', i);
					...
					a4_chesh = $rootScope.get_a4_chesh(ispause);
					if($rootScope.details_list.print_check ==1 ){
						printPrinterA4(a4_chesh);
						b=[];
						b[0]=a4_chesh;
						b[1]=a4_chesh;
						b[2]=a4_chesh;
					...
					document.getElementById('add_customer_m_phone').value = "";
					document.getElementById('add_customer_m_car').value = "";
					var a = [];
					l = 0;
					...
					if (ispause != 1 && ispause !=2) {
						$rootScope.send_cashes_db(pause_string,cash_numf);
	
						//create achlafa paper- copy and save it in documents copy table
						$rootScope.create_achlafa_chesh(cash_kupanum, cash_global,cash_numf);
			...
			$rootScope.save_final_tash = function (firstamount_withoutmaam, cash_global) {
				$rootScope.finalltash2["free_vat"] = $rootScope.finalltash2["free_vat"] + firstamount_withoutmaam;
				if (firstamount_withoutmaam > 0) {
					$rootScope.finalltash2["free_vat1"]["hova"] = $rootScope.finalltash2["free_vat1"]["hova"] + firstamount_withoutmaam;
				}
				else {
					$rootScope.finalltash2["free_vat1"]["zicuy"] = $rootScope.finalltash2["free_vat1"]["zicuy"] - firstamount_withoutmaam;
				}
				...
				for (var key in payments1) {
					if (payments1.hasOwnProperty(key))
						for (var j = 0; j < payments1[key].length; j++) {
			...
			$rootScope.send_cashes_db = function (pause_string,cash_numf) {
				if ($scope.cashes['cash_num' + cash_numf + pause_string]['cashierid'] == null && typeof $rootScope.SearchWorker.WorkerNum != 'undefined') {
					$scope.cashes['cash_num' + cash_numf + pause_string]['cashierid'] = $rootScope.SearchWorker.WorkerNum;
					$scope.cashes2 = {};
					$scope.cashes2['cash_num' + cash_numf] = $scope.cashes['cash_num' + cash_numf];
					tran = JSON.stringify($scope.cashes2);
					tran2 = $scope.cashes2['cash_num' + cash_numf];
				}
				$http.post('inc/transactions.php', $.param({
					stock: GetURLParameter.GetURLParameter('stock'),
					journal: journal,
			...
			$rootScope.create_achlafa_chesh = function (cash_kupanum, cash_global,cash_numf) {
				var b = [];
				var num2 = 31;
				for (var i = 1; i < 4; i++) {
					var l = 0;
					...
					if (i == 3) {
						...
						if ($rootScope.details_list.logo != "") { //lc 24/07/2016 add logo
							...
							// b[i][l] = $rootScope.txtCenter("http:kupa.yedatop.com/officefiles/" + user_name + "/_MlaitekPro/" + $rootScope.details_list.logo, i); //lc 21/07/2016
							b[i][l] = $rootScope.txtCenter("https://kupa.yedatop.com/officefiles/" + user_name + "/_MlaitekPro/" + $rootScope.details_list.logo, i); //lc 21/07/2016
							l++;
						}
					}
					...
					l++;
					b[i][l] = $rootScope.txtCenter(cash_kupanum.toString(), i);
					l++;
					...
					// b[i][l] = $rootScope.txtCenter('<img style="max-width:130px;width:130px;" src="https://office.yedatop.com/modules/stock/cashbox_fe/inc/barcode.php?height=50&barcode=' + $scope.cashes['cash_num' + cash_numf].cash_kupanum + '"/>', i);
					b[i][l] = $rootScope.txtCenter('<img style="max-width:130px;width:130px;" src="https://kupa.yedatop.com/modules/stock/cashbox_fe/inc/barcode.php?height=50&barcode=' + $scope.cashes['cash_num' + cash_numf].cash_kupanum + '"/>', i);
					l++;
					b[i][l] = "";
					l++;
					if (i == 3) {
		...
		function ($scope, $rootScope, cash_global, local) {
			...
			$rootScope.print = function () {
				...
				for (i = 1; i < 4; i++) {
					...
					a[i][l] = "";
					l++;
					a[i][l] = $rootScope.itemLine("סה\"כ תקבולים:", ($rootScope.finalltash2['cash'] * 1 + $rootScope.finalltash2['credit'] * 1 + $rootScope.finalltash2['cheque'] * 1 + $rootScope.finalltash2['shovar'] * 1).toFixed(2), i);
					l++;
					a[i][l] = $rootScope.itemLine("סה\"כ תקבולים חייבים מע\"מ:", ($rootScope.finalltash2['cash'] * 1 + $rootScope.finalltash2['credit'] * 1 + $rootScope.finalltash2['cheque'] * 1 + $rootScope.finalltash2['shovar'] * 1 - $rootScope.finalltash2['free_vat'] - ($rootScope.finalltash2['cash'] * 1 + $rootScope.finalltash2['credit'] * 1 + $rootScope.finalltash2['cheque'] * 1 + $rootScope.finalltash2['shovar'] * 1 - $rootScope.finalltash2['free_vat']) * 17 / 117).toFixed(2), i);
					l++;
					a[i][l] = $rootScope.itemLine("סה\"כ תקבולים פטורים מע\"מ:", ($rootScope.finalltash2['free_vat']).toFixed(2), i);
					l++;
					a[i][l] = $rootScope.itemLine("סה\"כ מע\"מ מתקבולים:", (($rootScope.finalltash2['cash'] * 1 + $rootScope.finalltash2['credit'] * 1 + $rootScope.finalltash2['cheque'] * 1 + $rootScope.finalltash2['shovar'] * 1 - $rootScope.finalltash2['free_vat']) * 17 / 117).toFixed(2), i);
					l++;
					a[i][l] = "";
					l++;
				}
