...
app.controller('PaymentController', ['$http', '$scope', '$rootScope', '$compile', 'tabs', 'local', 'cash_global', 'validate', 'alert', 'openwindows', function ($http, $scope, $rootScope, $compile, tabs, local, cash_global, validate, alert, openwindows) {
    ...
    if (localStorage.getItem('finalltash') === null) {
        $rootScope.finalltash = {
            ...
            "free_vat1": { "hova": 0, "zicuy": 0 },
            "shovarzicuy1": { "hova": 0, "zicuy": 0 },
            "shovarzicuy": 0,
            "points1": { "hova": 0, "zicuy": 0 },
            "points": 0,
            "akafadebt": {
                "general": 0,
                "cash": 0,
    ...
    $scope.isdebt = function (type) {
        if (type == 1) {
            ...
            if ($rootScope.transtype != 1) {
                openwrap2('', '.points.container_pop');
                $scope.init_sum();
            }
            else {
                $scope.call_alert_site('תשלום חוב ! לא מורשה תשלום בנקודות', 1, 0);
            }
        }
    }
    ...
    $rootScope.start_credix = function () {
        ...
        if (
            (typeof window.external != "undefined" && typeof window.external.startPayCredit != "undefined")
            || (typeof android2 != 'undefined' && typeof android2.startPayCredit != "undefined" )
            && Math.abs($rootScope.amount) > 0
        ) {
            writelog("test_startPayCredit198" + $rootScope.itra);
            if (typeof window.external != "undefined" && typeof window.external.startPayCredit != "undefined") {
                str = window.external.startPayCredit($rootScope.itra);
            }

            if (typeof android != 'undefined') {
                str = android.startPayCredit($rootScope.itra);
            }

            if (typeof android2 != 'undefined') {
                str = android2.startPayCredit($rootScope.itra);
            }

            writelog("test_תגובה מcredix: " + str);
            if (str.indexOf("error") < 0) {
                ...
                $scope.$apply();
                writelog("test_תגובה x: " + JSON.stringify($scope.payments_type['credit']));
                writelog("test_push x: " + JSON.stringify(x));
                writelog("test_הוספת תשלום באשראי: " + data_arr['amount']);
                paym.update_paid(parseFloat(data_arr['amount']));
                writelog("test_תגובה update_paid: " + parseFloat(data_arr['amount']));
                if ($rootScope.transtype == 1) {
    ...
    $rootScope.start_pay = function () {
        ...
        // console.log($rootScope.SearchWorker);
        // console.log('apply: ');
        // console.log($apply);
    ...
    $rootScope.start_pay111 = function (str) {
        ...
        if ($rootScope.itra) {
            ...
            if (str == ".points" && $rootScope.transtype == 1) {
                $scope.call_alert_site('תשלום חוב ! לא מורשה תשלום בנקודות', 1, 0);
            }
    ...
    $scope.start_ashray_emv = function () {

        console.log('start_ashray_emv() function START.')

        if ($(".input_num.ashray_text").val() != "") {

            $.ajax("inc/functions.php?get_emv_details=1&stock=" + stock, { async: false })
                .success(function (data) {

                    arr = JSON.parse(data);
                    var amount = $(".ashray_text").val();
                    ...
                    if (typeof android != 'undefined' && android != {}) {
                        console.log('android object: ')
                        console.log(JSON.stringify(android));
                        android.startPayCredit(JSON.stringify({
                            amount: amount,
                            IpayUserName: arr["IpayUserName"],
                            IpayPassword: arr["IpayPassword"],
                            pay_num: pay_num,
                            pay_first: pay_first,
                            ashray_f_credit: ashray_f_credit,
                            IpayCom: arr["IpayCom"],
                            aproval_num_f: $("input[name=aproval_num_f]").val()
                        }));
                    }

                    if (typeof android2 != 'undefined' && android != {}) {
                        console.log('android2 object: ');
                        console.log(JSON.stringify(android2));
                        android2.startPayCredit(JSON.stringify({
                            amount: amount,
                            IpayUserName: arr["IpayUserName"],
                            IpayPassword: arr["IpayPassword"],
                            pay_num: pay_num,
                    ...
                    if (typeof (window.external) != 'undefined' && typeof (window.external.startPayCredit) != 'undefined') {
                        writelog(amount + "," + arr["IpayUserName"] + "," + arr["IpayPassword"] + "," + pay_num + "," + pay_first + "," + ashray_f_credit + "," + arr["IpayCom"] + "," + $("input[name=aproval_num_f]").val());

                        var str = window.external.startPayCredit(amount, arr["IpayUserName"], arr["IpayPassword"], pay_num, pay_first, ashray_f_credit, arr["IpayCom"], $("input[name=aproval_num_f]").val());
                        writelog("test_תגובה מcredix: " + str);
                        data_arr = JSON.parse(str);
                        if (str.indexOf("error") < 0) {

                            if (data_arr['code'] == 60000) {
                                ashray_f_credit = 0;
                                data_arr = data_arr['data'];
                                data_arr = data_arr['transaction'];
                                writelog("test_data_arr:" + JSON.stringify(data_arr));
                                if (data_arr['creditTerms'] == "PAYMENTS") {
                                    ashray_f_credit = data_arr['numberOfPayments'];
                                }
                                /*var x={"type":'אשראי',"amount":data_arr['amount'],"pay_first":data_arr['firstPaymentAmount'],
                                ...
                                writelog("test_אשראי מחזיר שגיאה: " + str);
                                $("p.emvp").text("");
                                if (typeof data_arr['data']['message'] != 'undefined') {
                                    str2 = data_arr['code'] + " " + data_arr['data']['code'] + " " + data_arr['data']['message'];
                                }
                                else {
                                    if (typeof data_arr['error'] != 'undefined') {
                                        str2 = data_arr['error'];
                                    }
                                    else {
                                        str2 = str;
                                ...
                                $scope.call_alert_site(str2, 1, 'on');
                            }
                        }
                        else {
                            writelog("test_אשראי מחזיר שגיאה: " + str);
                    ...
                    writelog("test_התקשורת אינה תקינה ואשראי נפל");
        ...
        //alert('start_ashray_emv() function END.')
        ...
        if ($(str).val() != '') {
            switch (type) {
                case 'מזומן':
                    ...
                    writelog("test_הוספת תשלום במזומן: " + pay);
                ...
                case 'המחאה':
                    ...
                    var x = {
                        "type": 'המחאה', "amount": $(str).val(), "chequepaymentdate": date_cheque, "chequebanknumber": $('input[name=chequebanknumber]').val(), "chequebranch": $('input[name=chequebranch]').val(), "chequeaccountnumber": $('input[name=chequeaccountnumber]').val(), "chequenumber": $('input[name=chequenumber]').val()
                    ...
                    writelog("test_הוספת תשלום בהמחאה: " + $(str).val());
                    paym.update_paid($(str).val());
                    if ($rootScope.transtype == 1) {
                ...
                case 'אשראי':
                    ...
                    */
                    ...
                    $.ajax("inc/functions.php?ipay_emv=" + ipay_emv + "&amount=" + sum_c + "&track2=" + num_c + "&stock=" + stock + "&pay_num=" + pay_num + "&pay_first=" + pay_first + "&ashray_f_credit=" + ashray_f_credit + "&ashray_numcard=" + ashray_numcard + "&ashray_tokef=" + ashray_tokef + "&ashray_cvv=" + ashray_cvv + "&ashray_tz=" + ashray_tz + "&ApprovalNumber=" + $("input[name=aproval_num_f]").val(), { async: false })
                        .success(function (data) {
                            if (data.indexOf("error") < 0) {
                                data_arr = JSON.parse(data);

                                if (ipay_emv == 1) {
                                    data_arr['voucherNumber'] = data_arr['transactionNumber'];
                                    data_arr['firstPaymentAmount'] = data_arr['transaction']['firstPaymentAmount'];
                                    data_arr['numberOfPayments'] = data_arr['transaction']['numberOfPayments'];
                                    data_arr['amount'] = data_arr['transaction']['amount'];
                                    data_arr['cardNumber'] = data_arr['transaction']['cardNumber'];
                                    data_arr['cardExpirationDate'] = data_arr['transaction']['cardExpirationDate'];
                                    data_arr['creditCardCompanyIssuerCode'] = data_arr['transaction']['creditCardCompanyIssuerCode'];
                                    data_arr['cardBrand'] = data_arr['transaction']['cardBrand'];
                                }
                            }
                            ...
                            if (data.indexOf("error") < 0 && voucherNumberss.indexOf(data_arr['voucherNumber']) == -1) {
                                ...
                                writelog("test_================response data===========");
                                writelog("test_inc/functions.php?amount=" + sum_c + "&track2=" + num_c + "&stock=" + stock + "&pay_num=" + pay_num + "&pay_first=" + pay_first + "&ashray_f_credit=" + ashray_f_credit + "&ashray_numcard=" + ashray_numcard + "&ashray_tokef=" + ashray_tokef + "&ashray_cvv=" + ashray_cvv + "&ashray_tz=" + ashray_tz + "&ApprovalNumber=" + $("input[name=aproval_num_f]").val());
                                writelog(data);
                                writelog("test_================response data end===========");
                                ...
                                var x = {
                                    ...
                                    "pay_first": data_arr['firstPaymentAmount'],
                                    "pay_num": data_arr['numberOfPayments'],
                                    "ashray_numcard": data_arr['cardNumber'],
                                    "ashray_tokef": data_arr['cardExpirationDate'],
                                    "creditbrand": data_arr['cardBrand'],
                                    "approvalNumber": data_arr['voucherNumber'],
                                    "clearinghouseCompany": data_arr['creditCardCompanyIssuerCode'],
                                    "ashray_f_credit": ashray_f_credit
                                };
                                if (data_arr['amount'] < 0) {
                                    $rootScope.finalltash.credit2[data_arr['creditCardCompanyIssuerCode']]['CREDIT'] = (parseFloat($rootScope.finalltash.credit2[data_arr['creditCardCompanyIssuerCode']]['CREDIT']) + Math.abs(parseFloat(data_arr['amount']))).toFixed(2);
                                ...
                                if (data.indexOf("error") < 0 && ipay_emv == 1) {
                                    data_arr['voucherNumber'] = data_arr['transactionNumber'];
                                }
                                if (data.indexOf("error") < 0 && voucherNumberss.indexOf(data_arr['voucherNumber']) != -1) {
                                    $("textarea[name=ashray_f]").focus();
                                    ...
                                    $('textarea[name=ashray_f]').val("");
                                    writelog("test_יש להעביר אשראי שוב: " + data);
                                }
                                else {
                                    if (data.indexOf("התקשר") < 0) {
                                        ...
                                        $('textarea[name=ashray_f]').val("");
                                        writelog("test_אשראי מחזיר שגיאה: " + data);

                                    }
                                    else {
                                        str2 = "inc/functions.php?ipay_emv=" + ipay_emv + "&amount=" + sum_c + "&track2=" + num_c + "&pay_num=" + pay_num + "&pay_first=" + pay_first + "&ashray_f_credit=" + ashray_f_credit + "&ashray_numcard=" + ashray_numcard + "&ashray_tokef=" + ashray_tokef + "&ashray_cvv=" + ashray_cvv + "&ashray_tz=" + ashray_tz;
                                        $("input[name=credit_approval_details]").val(str2);
                                        openwindows.openwrap2('', '.credit_approval.container_pop');
                                        paym.init_sum2();
                                        $(".ashray_text").val("");
                                        writelog("test_אשראי דורש אישור");
                ...
                case 'אשראי2':
                    ...
                    $.ajax($("input[name=credit_approval_details]:hidden").val() + "&ApprovalNumber=" + $("input[name=credit_approval_num]").val(), { async: false })//send data to ipay
                        ...
                        .success(function (data) {
                            writelog("test_====================================");
                            writelog("test_אשראי________________" + data);
                            writelog("test_====================================");
                            if (data.indexOf("error") < 0) {
                ...
                case 'הקפה':
                    ...
                    writelog("test_הוספת תשלום בהקפה: " + $(str).val());
                ...
                case 'שובר':
                    ...
                    writelog("test_הוספת תשלום בשובר: " + $(str).val());
                    paym.update_paid($(str).val());
                    if ($rootScope.transtype == 1) {
                ...
                case 'שובר זיכוי':
                    ...
                    if ($(str).val() > 0) {
                        ...
                        if (is_exit != 0) {
                            ...
                            writelog("test_הוספת תשלום בשובר זיכוי: " + sum);
                            ...
                            if ($rootScope.itra < 0) {
                                ...
                                $scope.payments_type['shovarzicuy'].push(x);
                                writelog("test_הוספת תשלום בשובר זיכוי: " + $rootScope.itra);
                        ...
                        $scope.payments_type['shovarzicuy'].push(x);
                        writelog("test_הוספת תשלום בשובר זיכוי: " + $(str).val());
                        paym.update_paid($(str).val());
                    ...
                    if ($(str).val() > 0) {
                        ...
                        $.ajax("inc/functions.php?using_client_points="+$(str).val()+"&client_num="+$rootScope.SearchClient.ClientNum, { async: false })
                        .success(function (data) {
                        	data=data.split(";");
                            is_exit = data[0];
 pnts = data[1];
                        ...
                        if (is_exit != 0) {
                            ...
                            if (parseFloat(is_exit) > parseFloat($(str).val())) {
                                sum = parseFloat($(str).val()); 
                            }
                            else {
                                sum = is_exit;
                                //$rootScope.another_shovar=parseFloat(is_exit)-parseFloat($(str).val());
                            }
                            if(sum > $rootScope.itra){
                                sum = $rootScope.itra;
                            }
                            var x=$scope.payments_type['points'][0];
                            var pay = sum;
                ...
                $scope.payments_type['points']=[{"type":'נקודות',"amount":pay,"points":pnts}];                             
                           
                            paym.update_paid(sum);

                            if ($rootScope.transtype == 1) {
                                $rootScope.finalltash['akafadebt']['points'] = (parseFloat($scope.finalltash['akafadebt']['points']) + parseFloat(sum)).toFixed(2);
                                $rootScope.finalltash['akafadebt']['general'] = (parseFloat($scope.finalltash['akafadebt']['general']) + parseFloat($rootScope.finalltash['akafadebt']['points'])).toFixed(2);
                            }

                            $rootScope.finalltash['points1']['hova'] = (parseFloat($scope.finalltash['points1']['hova']) + parseFloat(sum)).toFixed(2);
                            $rootScope.finalltash['points'] = (parseFloat($scope.finalltash['points']) + parseFloat(sum)).toFixed(2);
                            //$rootScope.finalltash['all']=(parseFloat( $rootScope.finalltash['all'])+parseFloat(sum)) .toFixed(2); 
                            ...
                            /*sk 07/09 check if shovar has been exist in the acount */
                            if (is_exist_message) {
                                $scope.call_alert_site(' נא לבחור לקוח', 1, 0);
                                is_exist_message = 0;
                            }
                            else {
                                $scope.call_alert_site('אין נקודות למימוש', 1, 0);
                            }
                        }
                    }                    
                                    ...
                                    if (flag == 1) {
                                        ...
                                        for (i = 0; i < $scope.payments_type['prepaid'].length; i++) {
                                            if ($scope.payments_type['prepaid'][i].prepaid_num == num_p) {
                                                ...
                                                $scope.payments_type['prepaid'][i].amount = data[0];
                                                flag = 1;
                                            }
                                        }
                                        ...
                                        var x = { "type": 'פריפייד', "amount": data[0], "prepaid_num": num_p, 'balance': data[1] };
                                        $scope.payments_type['prepaid'].push(x);
                                        writelog("test_הוספת תשלום בפריפייד: " + data[0]);
                                        paym.update_paid(data[0]);
                                    }
