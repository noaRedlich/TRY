(function() {
 ...
 app.controller('SettingsController',[ '$scope', '$rootScope','$http','cash_global','local','alert','GetURLParameter',function ($scope,$rootScope,$http,cash_global,local,alert,GetURLParameter){
  ...
  if(print_check_li!=''){
  	print_check=print_check_li;
  }
  else{
  	print_check=details_list[0]['print_check'];
  }
	...
	$rootScope.prefers_list={
		...
		"pr_print_temp":(premission_list[0]['pr_print_temp']==1?'on':'off'),
		...
		"pr_viewpayment_points":(premission_list[0]['pr_viewpayment_points']==1?'on':'off'),
		...
		"pr_show_checkcheque_button":(premission_list[0]['show_checkcheque_button']==1?'on':'off'),
		...
		"pr_show_points_button":(premission_list[0]['show_points_button']==1?'on':'off'),
		"pr_weight_no_vat":(premission_list[0]['pr_weight_no_vat']==1?'on':'off'),
    ...
    $scope.save_prefers = function() {
    	...
    	$rootScope.prefers_list.pr_viewpayment_points=$(".pr_viewpayment_points").val();
    	...
    	$rootScope.prefers_list.check_points_button=$rootScope.prefers_list.pr_show_points_button=='on'?1:0;
