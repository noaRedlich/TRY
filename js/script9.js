...
$(document).ready(function(){ 
	if(typeof(window.external)!='undefined' && typeof(window.external.check_version)!='undefined'  ){
	   var type= window.external.check_version();
	 	...
	 	if(typeof(csharp)!='undefined' && typeof(csharp.check_version)!='undefined'  ){
		   var type= csharp.check_version();
		}
		else{
		 	var type='browser';
		 }
...
function writelog(desc){
		...
		if(typeof csharp!='undefined'&&typeof csharp.writelog!='undefined'){
			csharp.writelog(journal_id,desc);
		}
...
function check_cheque(add){
 
 jQuery(".check-cheque").addClass('load');
 var obj = {};
 var datas = jQuery(".amchaa .amchaa_frm").serializeArray();
 
obj.amount = datas[1].value;
obj.chequenumber = datas[2].value;
obj.chequepaymentdat = datas[3].value;
obj.chequebanknumber = datas[4].value;
obj.chequebranch = datas[5].value;
obj.chequeaccountnumber = datas[6].value;
obj.tashlumin = datas[7].value;

obj.identification = datas[8].value;
obj.phone = datas[9].value;
obj.garant = datas[10].value;
obj.businessID = datas[11].value;
jQuery(".confirmCode").hide();
jQuery(".error-message").removeClass('error success').text('');	
 jQuery.ajax({
            type: "POST",
            url: "https://kupa.yedatop.com/api/",
            data: obj,
            success: function (msg) {
              jQuery(".check-cheque").removeClass('load');
              msg = JSON.parse(msg);
              console.log(msg);
			//  window.alert(
			 if(msg.Confirmation){
			 	jQuery(".confirmCode").show();
			 	jQuery(".submitter, input[value='אישור']").attr('disabled',false);
			 	jQuery(".confirmCode").val(msg.Confirmation);
			 	setTimeout(function(){
			 			if(add) jQuery(".submitter").click();
			 				else 
			 				jQuery(".amchaa .allbottom .mybtn.btnblue").click();
			 			
			 	},3500);
			 			 	}
			 else if(msg.Success === 'FALSE'&&msg.status === "1")
			 {
			 	jQuery(".confirmCode").show();
			 	jQuery(".submitter, input[value='אישור']").attr('disabled',false);
			 	jQuery(".mecholel_amchaa2 .confirmCode").on('keyup',function(){
			 			
			 			jQuery(".confirmCode").val($(this).val())
			 	});
			 }
              if(msg.Success === 'FALSE'){
              	jQuery(".error-message").addClass('error');
              	jQuery(".error-message").text(msg.ErrMessage);
              }
              else {
              	jQuery(".error-message").addClass('success');
              	jQuery(".error-message").text(msg.stextfooter);
              }
              
              if(parseInt(msg.dealid)>0){
			  	jQuery(".error-message").text('מספר עסקה שלך '+msg.dealid+'. נא להתקשר למוקד');
			  	
			  	}
			  	else{
			  			jQuery(".error-message").text('מספר עסקה שלך NONE. נא להתקשר למוקד');
			  		}
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
...
$.ajax({
	...
	success: function(url) {
		...
		if (typeof android != 'undefined') {
			// $.ajax({
			// 	type: "POST",
			// 	url: "inc/writelog.php",
			// 	data: {
			// 		txt: 'android obj: ' + JSON.stringify(android) + ' --> isNull of undefined andoid obj: ' + (android ? 'yes' : 'no')
			// 	},
			// 	success: function(response) {
			// 		console.log('logs response: ', response)
			// 	},
			// 	error: function(XMLHttpRequest, textStatus, errorThrown) {
			// 		console.error('log error. textStatus: ', textStatus)
			// 	}
			// });
			android.onLogoLoaded(JSON.stringify({
				logoUrl: logoUrl
			}));
		} else {
				if (typeof android2 != 'undefined') {
					...
					// 	url: "inc/writelog.php",
					// 	data: {
					// 		txt: 'android2 obj: ' + JSON.stringify(android2) + ' --> isNull of undefined andoid2 obj: ' + (android2 ? 'yes' : 'no')
					// 	},
					// 	success: function(response) {
					// 		console.log('logs response: ', response)
					// 	},
					...
					// 	}
					// });
					android2.onLogoLoaded(JSON.stringify({
						logoUrl: logoUrl
					}));
				} else {
					console.log('hello 1');
						if (typeof window.external != 'undefined' && typeof window.external.onLogoLoaded != 'undefined') {
								window.external.onLogoLoaded(JSON.stringify({
									logoUrl: logoUrl
								}));
						}
						if (typeof csharp != 'undefined' && typeof csharp.onLogoLoaded != 'undefined') {
								csharp.onLogoLoaded(JSON.stringify({
									logoUrl: logoUrl
								}));
						}
				}
...
function printPrinter3(a, s) {//עם השהיה
    if (typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined') {
    	...
    	if (typeof csharp != 'undefined' && typeof csharp.print4 != 'undefined') {
	        writelog(json_html2(a[3]));
	    }
	    else {
    ...
    if (typeof android != 'undefined') {
        ...
        android.print_invoice(JSON.stringify(removeEmptyLines(a[2])), s, logoUrl);
        ...
        if (typeof android2 != 'undefined') {
            ...
            android2.print_invoice(JSON.stringify(a[1]), logoUrl);
        } else {
            if (typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined') {
...
function printPrinter4(a, barcode, s) {//עם השהיה+הדפסת ברקוד
	console.log('printPrinter4');
	console.log('a', a);
	console.log('a[1]', a[1]);
	console.log('a[2]', a[2]);
	a1withoutMultipleSpace = [];
	a[1].forEach(function(item, index) {
		a1withoutMultipleSpace.push(item.replace(/ +(?= )/g, ''));
	});
	a2withoutMultipleSpace = [];
	a[2].forEach(function(item, index) {
		a2withoutMultipleSpace.push(item.replace(/ +(?= )/g, ''));
	});
    c = setBarcode13(barcode);
    if ((typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined')||(typeof csharp != 'undefined' && typeof csharp != 'undefined')) {
        writelog(json_html2(a[3]));
    }
    else {
        writelog(JSON.stringify(a[3]));
    ...
    //var x=json_html(a);
    if (typeof android != 'undefined') {
        android.print_invoice2(JSON.stringify(removeEmptyLines(a2withoutMultipleSpace)), setBarcode13(barcode), s);

    } else {
        if (typeof android2 != 'undefined') {
            android2.print_invoice2(JSON.stringify(removeEmptyLines(a1withoutMultipleSpace)), setBarcode13(barcode));
        } else {
            if (typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined') {
                window.external.print4(json_html(a[3]));
            }
    ...
    b = {0: a1withoutMultipleSpace, 1: removeEmptyLines(a[2]), 2: json_html(a[3])};

    return b
}
/**
 * Print with correct json structure: title, products, payments, after_text
 * © Sergey 25/11/2018
 */
function printPrinterJsonStructure(aObject, barcode, s){
	console.log('printPrinterJsonStructure');
	console.log('->>>', aObject);
    if (typeof android != 'undefined') {
        android.print_invoice2(aObject, setBarcode13(barcode), s);
    } else if(typeof android2 != 'undefined') {
		android2.print_invoice2(aObject, setBarcode13(barcode));
    }
}
function printPrinter2(a, barcode) {
    ...
    //var x=json_html(a);
    if (typeof android != 'undefined') {
        android.print_invoice2(JSON.stringify(removeEmptyLines(a1withoutMultipleSpace)), setBarcode13(barcode), 0);
        ...
        if (typeof android2 != 'undefined') {
            ...
            if (typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined') {
            	...
            	if (typeof csharp != 'undefined' && typeof csharp != 'undefined') {
	                csharp.print4(json_html(a[3]));
	            }
	            else {
	                document.title = "app://print4#" + json_html2(a[3]);
...
function printPrinterA4(html){
	if (typeof android != 'undefined') {
        android.print_invoice2(html);

    } else {
        if (typeof android2 != 'undefined') {
            android2.print_invoice2(html);
        } else {
            if (typeof window.external != 'undefined' && typeof window.external.print4 != 'undefined') {
                window.external.print4(html);
            }
            else {
            	if (typeof csharp != 'undefined' && typeof csharp != 'undefined') {
	                csharp.print4(html);
	                ...
	                document.title = "app://print4#" + html;
	            }
            }
        }
    }
...
function printPrinter(a) {
	...
	//  console.log(window.external.print4);
    ...
    if (typeof android != 'undefined') {
				console.log('android object: ')
				console.log(JSON.stringify(android));
        android.print_invoice(JSON.stringify(removeEmptyLines(a[2])), 0, logoUrl);
...
/**
 * Return URL to "bussines" logo from system settings
 */
function getLogoUrl()
{
	return logoUrl;
