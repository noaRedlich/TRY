...
//$json_str=str_replace("'","",$json_str);
    ...
    return $json_str;
			...
			//$sql="select StockName,ID from vcx_weberp.listingsStocks where TerminalID=$stock and user_ID=$userID";
			 ...
			 $sql = "select StockName,ID,Mikud,Email,Fax,Phone,Address,name_for_cashbox,IpayEmv from vcx_weberp.listingsStocks where TerminalID=$stock and user_ID=$userID";//03/08 print name per stock
		...
		<div class="container_pop amchaa display">
			...
			<form class="amchaa_frm" style="width: 64%;margin: 0 auto;">
				...
				<div style="width:60%;float:left">
					...
					<input name="tashlumin" class="text required" />
					<input name="businessID"  type="hidden"  value="<?=$user['businessnum'];?>"/>
				</div>
				 <div class="check-cheque" onclick="check_cheque()">לבדוק המחאה</div>
...
<?php } ?>
			<input class="mybtn btnlightgray rightbottom" type="button" value="ביטול" onclick="openwrap2('','.type_pay.container_pop');$('.loading').css('display','none');">
		...
		<div class="container_pop mecholel_amchaa2 display">
			...
			<div class="allbottom">
				...
				<input class="mybtn btnorange " type="button" value="אופציות אשראי" style="visibility: hidden" onclick="init_sum()">
				<!--<input class="mybtn btngreen " type="button" value="בדיקת המחאה" onclick="openwrap2('','.mecholel.container_pop')">-->
				<input class="mybtn btnorange " type="button" value="חולל" ng-click="payment.cholel2();" >
			</div>
		</div>
  ...
  <div class="header" style="z-index: 99999">
   ...
   <div class="stat inline" style="width: 11%;">
        ...
        <label id="stock_name" data-id="<?=$stock?>"><?=$stock?></label>Of{{finalltash2['all']}}
   </div>
  			...
  			<style>
  				...
  				color:#a1d46f;
  				  ...
  				  .check-cheque.load:after{
  				  	 background:url('https://mir-s3-cdn-cf.behance.net/project_modules/disp/ab79a231234507.564a1d23814ef.gif') no-repeat center center / 32px;
  				  	 position:absolute;
  				  	 width:100%;
  				  	 height:100%;
  				  	 left:0;
		...
		<div  class="relative search_form ">
			...
			<input type="button" ng-click="search_c2(' .cust_search_value2');" style="display: none"  class="search_p_global main_search_cust2" value="חפש">
			   ...
			   <i class="fa fa-search-plus search_p_plus " ng-click="call_setTab(8)" style="float:right;margin-right: 1%;margin-left: 2%;"></i>
    					<!--sk 03/03/216-->
                        <input type="text" class="add_customer_m search_input"  style="flaot:left !important" id="add_customer_m" placeholder="לקוח מזדמן">
  		...
  		<table style="width: 100%;font-size:150%;  margin-top: 4%;">
  		...
  		<div class="details_area" id="cat2" style="width: 100%;overflow-y: scroll;  margin-top: 0;" >
				  ...
				  <tr class="active tr_{{cash_prd.BarCode}}" >
				  	...
				  	<td class="prod_SalePrice">{{::cash_prd.SalePrice}}</td>
							...
							<p class="plusminus" ng-click="call_setTab(1);call_setCountIndex(cash_prd.index);" style="display: inline-block;width:60px;text-align:center;">
								{{cash_prd.Amount}}
							</p><i class="fa fa-minus-circle" ng-show="cash_prd.Unit != 2" ng-click="cash_prod.minus_count((cash_prd.SalePrice-cash_prd.Discount),cash_prd.index,cash_prd.refund);"></i></td>
  		...
  		<div class="sum_area">
  			...
  			<div class="left inline">
  				<div class="newrow2"><!--here code 1-->
  					...
  					<div style="width: 100%;height: 26px;font-size: 130%;"><label class="right">17% מע"מ:</label><label class="left tax_sum zerofloat">{{amount_maam}}</label><label class="left">₪</label></div>
